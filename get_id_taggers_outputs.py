...
if __name__=='__main__':
    command = 'export PERL5LIB=/path_to_ensemble_tagger/ensemble_tagger/POSSE/Scripts'
    result = subprocess.run(command, shell=True)
    print(result.returncode)

    for file in os.listdir('./dataset/IDData/'):
        outputs = []
        df = pd.read_csv('./dataset/IDData/'+file)
        print(file)
        ...
        df.to_csv(f'et_out_{identifier_context}.csv')
