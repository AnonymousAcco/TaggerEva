...
from train import create_stanford_data, preprocess2spacy, read_stanford
from evaluation import nltk_pos_tag, spacy_pos_tag, stanza_pos_tag, flair_pos_tag, corenlp_pos_tag


def create_flair_data(data, mode='train'):
    with open('./dataset/ensemble_tagger_training_data/flair_format/et_flair_'+mode+'.txt', 'w') as file:
        for id in data:
            for word, tag in id:
                line = word + ' ' + tag + '\n'
                file.write(line)
...
def train_flair(train_data, test_data):
    ...
    data_folder = './dataset/ensemble_tagger_training_data/flair_format/'
...
def eva_flair(identifiers, tags):
    tagger = Classifier.load('./et_flair_tagger/best-model.pt')
    print(tagger)
...
def read_et_data(path):
    ...
    # pairs = [(str(word).lower(), tag) for word, tag in zip(words, tags)]
...
def test_spacy(ids, poses, model_path='./model/model-best'):
...
if __name__ == '__main__':
    ...
    print('CoreNLP:')
    corenlp_out = read_stanford('./etdata_stanford_out.txt')
    cal_acc_em(corenlp_out, test_labels_str)
    # print(test_identifiers, test_labels)
    eva_flair(test_identifiers, test_labels)
    ...
    # nltk_outs = nltk_pos_tag(test_identifiers, test_labels)
    # nltk_et_outs = map2et(nltk_outs)

    # print('NLTK:')
    # cal_acc_em(nltk_et_outs, test_labels_str)
    #
    # flair_outs = flair_pos_tag(test_identifiers, test_labels)
    # flair_et_outs = map2et(flair_outs)
    # cal_acc_em(flair_et_outs, test_labels_str)

    # stanza_outs = stanza_pos_tag(test_identifiers, test_labels)
    # stanza_et_outs = map2et(stanza_outs)
    # cal_acc_em(stanza_et_outs, test_labels_str)

    # test_ids_str = [' '.join(line) for line in test_identifiers]
    print('spaCy:')
