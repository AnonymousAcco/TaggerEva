...
def swum_posse_results(path='./out_data_et.csv', golden_path='./evaluation.csv', tagger='SWUM'):
    w_num = 0
    df = pd.read_csv(path)
...
if __name__ == '__main__':
    ...
    #     print(out_file)
    #     file_path = path + out_file + '.csv'
    #     golden_path = './evaluation_' + out_file + '.csv'
    #     for tagger in taggers:
    #         swum_posse_results(file_path, golden_path, tagger)

    # swum_posse_results(tagger='SWUM')
    # swum_posse_results(tagger='STANFORD')
    # swum_posse_results(tagger='POSSE')
    # swum_posse_results(tagger='OUT')
    # create_posse_data(config)


    # nl2et
    out_files = ['method', 'args', 'class', 'all']

    taggers = ['nltk', 'stanford', 'spacy', 'flair', 'opennlp', 'stanza']
    for tagger in taggers:
        print(tagger)
        for mode in out_files:
            print(mode)
            nl_tagger2et(tagger, mode)
